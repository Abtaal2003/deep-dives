...
Everything so far is the classic, manual workflow: create an environment with `venv`, install with `pip`, and track dependencies in `requirements.txt`. The modern tool **uv** compresses that entire workflow into one fast program, and it changes which files are authoritative.
...
uv makes `pyproject.toml` the single source of truth for dependencies (the same `[project]` and `[dependency-groups]` tables from Section 10), and it generates a `uv.lock` file that pins the exact version of every direct and transitive dependency for perfect reproducibility. The `.venv` is still there, still disposable, still gitignored, but you rarely create or activate it by hand: `uv run` and `uv sync` manage it for you.

The `requirements.txt` file becomes optional, produced only when something downstream needs it:
...
The full uv workflow, every command and the files it generates, is covered in depth in the companion guide, [The uv Project Lifecycle](../tooling/uv-project-lifecycle.md).
